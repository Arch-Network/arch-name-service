 ...
 *
 * Ported from arch-swap-engine's `extractSignature` /
 * `getWalletWitnessSignatureItem` path (verified with Xverse and UniSat).
 * LaserEyes ChallengeSigners return a BIP-322 simple witness blob (base64 or
 * hex); ANS's runner and local verifier need the inner 64-byte (r||s) hex.
 */
